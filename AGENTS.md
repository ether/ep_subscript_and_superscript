# Agent Guide — ep_subscript_and_superscript
...
`ep_subscript_and_superscript` runs inside Etherpad's test harness. From an etherpad checkout that has installed this plugin via `pnpm run plugins i --path ../ep_subscript_and_superscript`:
...
* Server: `eejsBlock_editbarMenuLeft`, `collectContentPre`, `collectContentPost`, `eejsBlock_dd_format`, `aceAttribClasses`, `exportHtmlAdditionalTags`, `asyncLineHTMLForExport`, `padInitToolbar`
* Client: `postAceInit`, `aceRegisterBlockElements`, `aceAttribsToClasses`, `aceAttribClasses`, `collectContentPre`, `collectContentPost`, `aceEditEvent`
