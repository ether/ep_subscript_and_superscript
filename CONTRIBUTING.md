# Contributing to ep_subscript_and_superscript

Thanks for helping improve `ep_subscript_and_superscript` — a plugin for [Etherpad](https://etherpad.org/). LLM/Agent contributions are explicitly welcome, provided they follow the rules below.
...
`ep_subscript_and_superscript` is a plugin, so you develop it inside a checkout of [ether/etherpad](https://github.com/ether/etherpad).
...
# Install ep_subscript_and_superscript from your local clone
pnpm run plugins i --path ../ep_subscript_and_superscript

# Start the dev server (port 9001)
pnpm --filter ep_etherpad-lite run dev
```

Edits in `../ep_subscript_and_superscript/` are picked up after a server restart.
...
* Etherpad version, ep_subscript_and_superscript version, browser + OS
* Steps to reproduce
